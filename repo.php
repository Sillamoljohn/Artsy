    ...
    <body>
      ...
      <table border="1" align="center" cellpadding="10px" style="font-size:22px; border-collapse: collapse; border: 4px solid green;">
                    <tr>
                        ...
                        <th>User Name &nbsp&nbsp&nbsp </th>
						<th>artName &nbsp&nbsp&nbsp </th>
                        <th>Amount &nbsp&nbsp&nbsp </th>
                        <th></th>
                    </tr>
					<?php
					$result =mysqli_query($con,"
SELECT `aregistration`.`fname`,`aregistration`.`lname`,
			`bid`.`amount`,`auctionart`.`sartname`,`auctionart`.`sart_id` FROM `auctionart`,`bid`,`aregistration` WHERE
			`bid`.`sart_id`=`auctionart`.`sart_id` AND`aregistration`.`areg`=`auctionart`.`areg` AND `auctionart`.`status`='0';");
