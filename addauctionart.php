...
$f=$_POST["sprice"];
 ...
 $j="sartimg/".time()."".htmlspecialchars($_FILES['sartimg']['name']);
               move_uploaded_file($_FILES['sartimg']['tmp_name'], $j); 
			   
$sql1="INSERT INTO `auctionart`( `sartname`, `sartdetails`, `sartimg`, `ssize`, `sprice`, `artcat_id`, `ureg` )  VALUES ('$b','$d','$j','$e','$f','$c','$ureg')";
...
$result1=mysqli_query($con,$sql1);	
	...
	header('location:addauctionart.php');	
} 
...
.button:active {
 ...
 <form name="form1" action="#" method="POST" id="form1" enctype="multipart/form-data" onsubmit="return addUser()">
	                	...
	                	<input type="text" name="sartname" id="sartname" placeholder="Enter Artname..." class="contact-email form-control" id="contact-email" required onChange="return Art()" >
	                        </div>
...
<?php
     ...
     ?>
	          	...
	          	<textarea name="sartdetails" id="sartdetails" rows="3" cols="5" placeholder="Enter Art Details..." class="contact-subject form-control"required onChange="return Artd()" ></textarea>
	                        </div> 
							
	 <div class="form-group">
	 <label for="art_size">Size [length x breadth]</label>
	<input type="text" name="ssize" id="ssize" placeholder="Enter Art size..." class="contact-message form-control">
    </div>
	<div class="form-group">
	<label for="art_price">Price</label>
	<input type="number" min="1000" name="sprice" id="sprice" placeholder="Enter art price..." class="contact-subject form-control" id="contact-subject" onkeypress="javascript:return isNumber(event)">
	
    </div>
	
	                    <div>	<label for="contact_image"> Upload Your ArtWork: </label>
   <input type="file" name="sartimg" id="sartimg"   required onchange="fileCheckk(this)"></div>
